...
extension CredentialPreviewInfo {
    internal init(using info: InvitationPreviewInfo) {
        ...
        if let jsonRepresentation = info.jsonRepresentation, let data = try? JSONDecoder().decode([String: AnyCodable].self, from: jsonRepresentation), let formats = info.formats {
            ...
            if formats.allSatisfy(["mso_mdoc", "mso_mdoc_detail", "mso_mdoc_preview"].contains) {
            ...
            else if formats.allSatisfy(["aries/ld-proof-vc-detail@v1.0"].contains) {
            ...
            else if formats.allSatisfy(["hlindy-zkp-v1.0"].contains) {
